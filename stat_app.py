...
analysis_purpose = st.sidebar.radio(
    "どちらの解析を行いますか？",["📊 要因解析 (カテゴリ比較・多重比較)", "📈 回帰解析 (数値からの予測・回帰曲線)"],
    help="グループ間の違いを見たいなら「要因解析」、温度や量から結果を予測したいなら「回帰解析」を選びます。"
)

st.sidebar.header("⚙️ 3. 目的変数の性質 (データ分布)")
mode = st.sidebar.selectbox(
    "データの種類",["📦 連続値：収量・草丈・重量など (正規分布/OLS)", 
...
# データがアップロードされていない場合はここで処理を停止し、メッセージを表示
if df is None:
...
c1, c2 = st.columns(2)
with c1:
    st.markdown("**▼ データプレビュー (先頭5行)**")
    st.dataframe(df.head(5), use_container_width=True)
with c2:
    st.markdown("**▼ データの要約 (Summary)**")
    # 数値以外の列もNaNを空文字にして綺麗に表示
    st.dataframe(df.describe(include='all').fillna(""), use_container_width=True)

st.divider()

st.markdown(f"### ⚙️ 2. 分析変数の設定 (設定：{analysis_purpose.split()[1]} × {mode.split()[1]})")
...
if st.button("🚀 解析を実行", type="primary", use_container_width=True):
    ...
    plt.rcParams['font.family'] = 'IPAexGothic'  # Windows環境等は適宜変更 (例: 'Meiryo')
